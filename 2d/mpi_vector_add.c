...
double Parallel_vector_point(double local_x[], double local_y[], int local_n);

void Parallel_vector_scalar(double local_v[], int local_n, double scalar);
/*-------------------------------------------------------------------*/
int main(void) {
   int n, local_n;
   int comm_sz, my_rank;
   double *local_x, *local_y, *local_z, partial_point, final_point;
   MPI_Comm comm;
   double tstart, tend;
   ...
   partial_point = Parallel_vector_point(local_x, local_y, local_n);
   MPI_Reduce(&partial_point, &final_point, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   ...
   if(my_rank==0) {
      printf("\n\nTook %f ms to run\n", (tend-tstart));
      printf("Producto punto: %lf", final_point);
   }
...
/*-------------------------------------------------------------------
 * Function:  Parallel_vector_point
 * Purpose:   Calculate the product point between two vectors
 * In args:   local_x:  local storage of one of the vectors being operated
 *            local_y:  local storage for the second vector being operated
 *            local_n:  the number of components in local_x, local_y,
 *                      and local_z
 * Out arg:   local_result:  local storage for the point product of the vectors
 */
double Parallel_vector_point(
      double  local_x[]  /* in  */,
      double  local_y[]  /* in  */,
      int     local_n    /* in  */) {
   int local_i;
   ...
   // *result_point = local_result;
   // printf("Result actual %f ", local_result);
